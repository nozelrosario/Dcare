    ...
    this.getDataByID = function (id) {
        ...
        if (id && id > 0) {
            this.getDataStore().get(id.toString()).then(function (data) {
                ...
                $log.error("DataStore.getDataByID : error occured while querying " + me.dataStoreName + "[Error: " + err + "]");
    ...
    */
    ...
    this.query = function (query) {
        ...
        this.getDataStore().gql(query, function (err, data) {
            if (!err) {
                if (data.rows) {
                    deferredQuery.resolve(rows);
                } else {
                    deferredQuery.resolve(null);
                }

            } else {
                $log.error("DataStore.query : error occured while querying" + this.dataStoreName + "[Error: " + err + "]");
                deferredQuery.resolve(null);
            }
        });
        return deferredQuery.promise;
    ...
    this.find = function (query) {
        ...
        this.getDataStore().createIndex({
            ...
            return me.getDataStore().find({
                ...
                $log.error("DataStore.find : error occured while querying" + this.dataStoreName + "[Error: " + err + "]");
                deferredQuery.resolve(null);
            });
        }).catch(function (err) {
            $log.error("DataStore.find : error occured while creating Index" + this.dataStoreName + "[Error: " + err + "]");
            deferredQuery.resolve(null);
        });
        return deferredQuery.promise;
    };
